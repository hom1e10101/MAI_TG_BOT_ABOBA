...
def handle_location(message, prev_message):
    ...
    tb.send_message(chat_id=message.chat.id, text = "YandexGPT анализирует данные и ищет интересные места поблизости...")
    ...
    try:
        ...
        if places_result and places_result.get('features'):
            ...
            # Send the results
            tb.edit_message_text(response_text, chat_id=message.chat.id, message_id=message.id, parse_mode="Markdown", disable_web_page_preview=True)
        else:
            # tb.delete_message(user_id, status_message.message_id)
