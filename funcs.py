...
def place(message, prev_message):
  ...
  if message.text == 'случайно' or message.text == 'Случайно':
    ...
    markup.add(button)
    tb.delete_message(message.chat.id, (message.message_id - 1))
    tb.send_message(user_id, "Пожалуйста, поделитесь своим местоположением:", reply_markup=markup)
    print(message.id)
    #tb.delete_message(message.chat.id, message.id)
    #tb.edit_message_text("", chat_id=message.chat.id, message_id=prev_message.message_id)