...
from places_requests import get_name_by_place_id
...
def create_navigation_keyboard_for_comments(user_id, current_index, total_comments, place_idx, comment_id):
    ...
    row2 = [InlineKeyboardButton("Назад", callback_data=f"back_{place_idx}")]
    markup.row(*row2)

    with get_db_connection() as conn:
        role = get_user_role(conn, user_id)
    if role is not None and role in ["admin", "moderator"]:
        row3 = [InlineKeyboardButton("Удалить", callback_data=f"rem_{comment_id}")]
        markup.row(*row3)
    return markup
