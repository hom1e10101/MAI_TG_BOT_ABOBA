...
from settings_requests import get_user_message_to_edit, upd_user_message_to_edit, get_user_city, upd_user_city, get_user_distance, upd_user_distance, get_user_last_request, upd_user_last_request
...
def handle_location(message):
    ...
    tb.edit_message_text(f"🔍 Запрашиваю у YandexGPT информацию о местах по запросу '{user_request}'...", chat_id=message.chat.id, message_id=prev_message)
    try:
        ...
        if places_result and places_result.get('features'):
            ...
            tb.edit_message_text(response_text, chat_id=message.chat.id, message_id=prev_message, parse_mode="Markdown", disable_web_page_preview=True)
        else:
